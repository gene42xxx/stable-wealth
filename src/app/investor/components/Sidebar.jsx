    ...
    <>
      ...
      <motion.aside
        ...
        className="
          ...
          bg-[rgba(17,17,23,0.85)] backdrop-blur-xl
          border-r border-white/10 shadow-2xl
          overflow-hidden flex flex-col flex-shrink-0"
      >
        {/* Animated gradient layers */}
        <div className="absolute inset-0 bg-gradient-to-br from-indigo-500/10 via-purple-500/05 to-pink-500/10 pointer-events-none" />
        ...
        <div className="relative z-10 flex items-center justify-between px-6 py-5 border-b border-white/10">
          <Link href="/dashboard" className="group flex items-center">
            <Image src="/sb.png" alt="Logo" width={160} height={40} priority className="h-12 w-auto transition-transform duration-300 group-hover:scale-105" />
            <motion.div className="absolute inset-0 bg-gradient-to-r from-indigo-400/20 to-purple-400/20 blur-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
          </Link>
          <button
            onClick={toggleMobileSidebar}
          ...
          >
            <X size={20} />
            <motion.div className="absolute inset-0 bg-gradient-to-br from-red-500/20 to-pink-500/20 opacity-0 group-hover:opacity-100 rounded-xl transition-opacity duration-300" />
          </button>
        </div>
              ...
              >
                <Link href={item.href} className="group block">
                  <div className={`
                    ...
                    ${active
                      ? 'bg-gradient-to-r from-indigo-600/30 via-purple-600/20 to-pink-600/30 text-white border border-white/20 shadow-lg'
                      : 'text-white/70 hover:text-white hover:bg-white/5 border border-transparent hover:border-white/10'}
                  `}>
                    {/* Active highlight */}
                    {active && (
                      <>
                        <motion.div layoutId="active-indicator" className="absolute left-0 top-2 bottom-2 w-1 bg-gradient-to-b from-indigo-400 via-purple-500 to-pink-500 rounded-r-full shadow-lg" />
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-0 bg-gradient-to-r from-indigo-500/10 via-purple-500/5 to-pink-500/10 rounded-2xl" />
                      </>
                    )}
                    ...
                    <motion.div
                      ...
                      className="absolute inset-0 bg-gradient-to-r from-indigo-400/10 via-purple-400/10 to-pink-400/10 rounded-2xl blur-sm"
                    />
                    ...
                    <div className="relative z-10 flex items-center">
                      ...
                      {active && <div className="w-1.5 h-1.5 bg-gradient-to-r from-indigo-400 to-purple-400 rounded-full" />}
                    </div>
                  </div>
                </Link>
              </motion.div>
        ...
        <div className="relative z-10 px-4 py-4 border-t border-white/10 bg-gradient-to-t from-white/5 to-transparent">
          ...
          >
            <div className="flex items-center space-x-4 overflow-hidden">
              <div className="relative w-10 h-10 rounded-full bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 flex items-center justify-center text-white font-bold text-lg shadow-lg">
          ...
          <AnimatePresence>
            {profileOpen && (
              ...
              >
                <Link
                  href={(userInfo.role === 'admin' || userInfo.role === 'super-admin') ? '/admin/settings' : '/dashboard/settings'}
        ...
        {/* Floating subtle pulses */}
        <div className="absolute top-20 right-4 w-1 h-1 bg-indigo-400/30 rounded-full animate-pulse" />
        <div className="absolute top-32 right-8 w-0.5 h-0.5 bg-purple-400/40 rounded-full animate-pulse delay-1000" />
        <div className="absolute bottom-40 right-6 w-1.5 h-1.5 bg-pink-400/20 rounded-full animate-pulse delay-2000" />
